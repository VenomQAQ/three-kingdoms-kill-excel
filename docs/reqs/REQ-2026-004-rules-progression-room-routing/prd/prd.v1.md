...
以下语义为本需求的统一口径，前端与后端必须共用，不得各自扩展解释：

| 状态/事件 | 语义 |
|---|---|
| `waiting` | 房间可复用的待命态。可加入、可准备、可由房主开始游戏。游戏结束后也回到该态。 |
| `selecting` | 正式房选将态。房间仍归同一批玩家，展示候选武将与倒计时，允许提交选择。 |
| `playing` | 对局进行态。卡牌、技能、结算与聊天都在该态下发生。 |
| `finished` | 一局刚结束但尚未完成状态回收的过渡态，仅用于结算广播和同步；随后必须回到 `waiting`。 |
| `connected` | 玩家在线、可接收状态广播。 |
| `disconnected` | 玩家掉线、关闭浏览器或网络中断后的保坐状态，不视为主动退出。 |
| `host` | 当前房间主控者。房主离开时需立即轮转给下一位可接管的真实玩家。 |
| `托管` | 玩家在 `selecting/playing` 期间掉线后的恢复语义，座位保留、金币不扣、可在重连后继续回到房间。 |
| `主动退出` | 玩家在 `selecting/playing` 中点击确认离开，触发扣 5 金币（最低到 0）并移出房间。 |

### 4.4 规则与版本

| 编号 | 需求点 | 优先级 | 备注 |
|---|---|---|---|
| R-RULE-01 | 建立规则 SSOT：以 `docs/cards/*.md`、`docs/gameplay.md` 与 `packages/shared/src/versions.ts` 为准，缺失或冲突处以官方/通行规则为准 | P0 | PRD 不绑定具体技术实现 |
...
> 版本口径统一：`standard-2014` 当前固定为 **2-8 人**，前后端、房间创建、房间列表与文案全部按此执行。
...
- **房间列表 Sheet**：只承载房间列表、版本入口、加入/返回动作和大厅相关信息。
- **当前房间 Sheet**：只承载等待、选将、对局、结算后的等待态，以及该房间内的操作。
- **区域销售 Sheet**：只承载伪装表格与与游戏无关的展示，不出现任何游戏动作。
- Sheet 切换的唯一标准是当前视图归属，不以是否在房间内、是否在对局中来推断页面身份。
- 页面壳允许刷新恢复，但恢复后必须回到与当前房间状态一致的 Sheet。
...
- `waiting`：直接离开；若是房主，立即轮转；若无人则解散。
- `selecting`：展示确认弹窗，说明「确认离开将扣除 5 金币，最低扣至 0；取消则继续留在房间」；离开后进入托管/保坐语义，不改结算结果。
- `playing`：展示确认弹窗，说明「确认离开将扣除 5 金币，最低扣至 0；取消则继续留在房间」；离开后进入托管/保坐语义，不改结算结果。
- `disconnected`：关闭页面、掉线或网络中断，不扣金币，进入托管/保坐流程。
- `托管`：房间状态保持不变，座位保留，等待重连或保坐超时后的后续处理；本轮只定义语义，不额外引入新 UI。
- 房主主动离开时，不等待保坐期，立即轮转给下一位可接管的真实玩家；若无可接管玩家，则解散。
...
- 选将阶段全员同时选择，最长 3 分钟。
- 候选列表顺序由服务端固定下发，超时未选时，服务端默认选择该玩家候选列表第一名。
...
- 本轮不做连续签到奖励。
- 本轮不做等级加入门槛，仅预留后续扩展。
- 成长经济（R-PROG-01~04）仅保留在文档中，不纳入本轮主验收与设计开发优先级。
...
- 主要条件：`R-SHEET-01` 需要和现有 Sheet/Tab 结构对齐，避免与 `sheet1/sheet2/game` 三层路由继续混用；`R-UI-07` 只读版本详情需要明确入口样式与返回路径。
- 风险点：房间列表、当前房间与区域销售三 Sheet 的初始边界清晰，但实际实现仍依赖现有 `Ribbon` 与 `SheetTabs` 重构。

### 后端可行性

- 结论：pass-with-conditions。
- 主要条件：`R-RULE-01` 的 `docs/cards/*.md` SSOT 目前在仓库中不存在，需要先补齐或改成明确的规则目录；`R-ROOM-06` 的 2-8 人限制与现有 `standard-2014` 版本定义冲突，必须先决定是改版本配置还是改 PRD；`R-RULE-09` 的选将超时默认候选第一名需要明确候选稳定顺序。
- 风险点：正式房循环与规则结算都落在同一套 Room/Engine 边界内，阶段拆分前要先冻结版本门槛。

### QA 可测性

- 结论：pass-with-conditions。
- 主要条件：`R-RULE-01`、`R-RULE-05`、`R-RULE-07` 需要补齐可测判据的规则出处；`R-ROOM-04` 的扣费确认文案已足够，但 `R-ROOM-05` 的托管/保坐需要明确可观察状态；`R-PROG-01~04` 属延期设计，需从当前三审验收中拆出。
- 风险点：规则敏感项多，若不先锁定 SSOT 和版本上限，后续用例会出现“测得出来但判不清”的问题。
