...
- **SSOT 声明**：自本文档 v2 定稿并通过三审复核起，**本轮功能流程以本文档为准**，直至产品下次显式更新 PRD。v1 保留只读。
...
5. **选将流程缺失**：正式房开局直接随机分配武将，未按三国杀规则进入「身份分配 → 选将 → 发起始手牌」流程。
6. **房间聊天不便**：房间内聊天仅能通过公式栏发送，聊天区下方无输入框（大厅聊天区已有）。

### 1.2 价值主张

让玩家在 WPS 伪装壳内获得**可恢复、可辨认、规则正确、视觉完整**的联机身份局体验，同时保留测试房快速构造对局的调试效率。

### 1.3 成功指标

| 指标 | 目标 |
|---|---|
| 刷新恢复 | 登录用户在 `reconnectGraceSec`（默认 5min）内刷新，**100%** 自动回到原房间视图（等待、选将或进行中） |
| 误回大厅重进 | 玩家从房间 Sheet 切回大厅后，**≤2 次点击**（房间列表找到本房 → 加入/返回）回到原座位 |
| 昵称一致性 | 首页、房间列表、聊天、大厅表格、状态栏等对外玩家标识 **100% 使用账号昵称**；QQ 邮箱仅用于账号表单 |
| 对局日志可读性 | 对局日志主要使用武将名描述动作，例如「关羽使用【杀】」；必要时可附昵称辅助区分 |
| 南蛮/万箭闭环 | 【南蛮入侵】【万箭齐发】在「无懈询问链结束」后，**所有存活目标均收到响应询问**，无卡死 |
| 对局区铺满 | 游戏 Sheet 三档分辨率（1440×900 / 1920×1080 / 2560×1440）非背景色像素占视口 **≤ 3%** |
| 选将完成率 | 正式房开局后全员在 **180s** 内完成选将（超时由服务端默认选候选列表第一张） |
...
| 老玩家 | 希望改昵称，且大厅、聊天、房间列表等处显示昵称而非邮箱 |
| 出牌玩家 | 打出【南蛮入侵】，期望所有目标依次响应【杀】 |
| 房主 | 正式房点击开始后，按三国杀流程选将再进入回合 |
| 测试者 | 在测试房自定义角色/武将，快速构造锦囊、装备、濒死等测试场景 |
...
| 编号 | 需求点 | 优先级 | 备注 |
|---|---|---|---|
| R-UX-01 | **刷新保持视图**：刷新后自动恢复房间上下文（等待大厅、选将或对局 Sheet），不回到纯大厅首页 | P0 | 依赖断线保坐 |
| R-UX-02 | **误回大厅可重进**：房间列表展示本人所在/最近房间，可点击重新进入（保坐期内免输房间号） | P0 | 与 R-UX-01 配套 |
| R-ACCT-01 | **全局展示昵称**：首页、房间列表、聊天、大厅表格、状态栏等面向玩家的身份文案显示**账号昵称**；QQ 邮箱仅用于登录/注册/账号表单 | P0 | 对局日志见 R-ACCT-04 |
| R-ACCT-02 | **编辑昵称**：已登录用户可修改昵称（Ribbon 或账号菜单入口），改后全局即时同步 | P1 | 2–12 字，禁纯空白 |
| R-ACCT-03 | **注册二次确认密码**：注册表单增加「确认密码」，两次不一致禁止提交并提示 | P0 | |
| R-ACCT-04 | **对局日志使用武将名**：对局动作日志主要使用武将名描述动作；多人同武将或需要身份确认时可附昵称 | P0 | 例：`关羽使用【杀】` |
...
| R-GAME-01 | **正式房选将阶段**：正式房房主开始游戏后，先分配身份，再进入选将阶段（非直接随机武将） | P0 | 见 §4.5 |
| R-GAME-02 | **选将数量**：主公 **5 将选 1**；其他角色 **3 将选 1**；按座次从主公起依次选 | P0 | 标准身份局规则 |
| R-GAME-03 | **选将信息展示与超时**：候选武将展示名称、技能、体力/血量；选将最长 180s，超时默认候选列表第一张 | P0 | 服务端权威 |
| R-SBX-01 | **测试房保留原逻辑**：测试房不强制进入正式选将流程，继续允许自定义角色/武将以便测试 | P0 | 正式房与测试房明确分流 |
...
- 刷新后加载顺序：`hydrate 登录` → `socket 连接` → 若 sessionStorage 有 roomCode 且用户已登录 → 自动 `room:join` → 恢复 `activeSheet` 为游戏 Sheet。
- 不新增产品必需的 `room:rejoin`；重进房行为统一复用 `room:join`。

**服务端**

- 保坐期内（`reconnectGraceSec`，默认 300s）：玩家座位保留，`room:join` 同 userId + code 命中原座位则静默重绑，不新增座位。
- 对局进行中重进：下发过滤后的 `room:state` + 必要时 `game:sync`，恢复当前 prompt。
- 选将中重进：下发 `room:state.status='selecting'` 与当前选将 prompt，恢复候选列表和剩余时间。

**误回大厅（R-UX-02）**

- 房间列表增加视觉提示：本人所在房间行高亮（如浅蓝底）+ 「等待中」/「选将中」/「进行中」状态列。
...
### 4.2 昵称体系与对局日志（R-ACCT-01 / R-ACCT-02 / R-ACCT-04）
...
| 大厅等待表格「昵称」列 | 昵称 | 邮箱 |
| 状态栏当前用户 | 昵称 | 邮箱 |
| 对局日志「玩家动作」 | 武将名为主，可按需附昵称：`关羽（阿明）使用【杀】` | 单独用邮箱 |
| 公式栏账号相关提示 | 昵称；账号表单内可出现邮箱 | 非账号场景展示邮箱 |
| 武将技能弹窗标题 | 武将名 + 技能 | — |
...
### 4.5 正式房选将流程（R-GAME-01 / R-GAME-02 / R-GAME-03）
...
- 产品指定房间主状态使用 `room.status='selecting'` 表示正式房选将中。
- 前后端 design 可追加子字段描述当前选将座次、倒计时、候选列表，但不得以仅客户端本地状态替代服务端权威状态。
...
5. 每名玩家最长选将时间 **180 秒**。超时未选时，服务端自动选择该玩家候选列表的第一张（`index=0`）。
6. 全员选毕后：
   - 设置体力上限（主公 +1）
   - 分发起始手牌 4 张
   - `room.status` → `playing`，引擎 `start()`，主公回合开始。

**UI 要点**

- 选将面板必须展示：武将名、体力/血量、技能名与技能摘要。
- 他人选将中：显示「等待 XXX 选将」（XXX 为昵称）。
- 已选武将：对全员可见（武将名），身份仍按规则隐藏。
- 候选武将列表由服务端权威下发；客户端不自行 shuffle、不自行补齐候选。

### 4.6 测试房保留原逻辑（R-SBX-01）

- 测试房（sandbox）不默认强制进入正式房选将流程。
- 保留现有自定义角色/武将能力，用于快速构造测试场景。
- 测试房「模拟开局」继续沿用现有快捷路径；如玩家已指定武将，服务端直接使用该武将进入测试流程。
- 正式房选将改造不得破坏 `sandbox:addPlayer`、手动指定武将、快速触发锦囊/装备/濒死场景等测试能力。
...
- 武将池：当前版本 `standard-2014` 已实装武将全集；正式房选将时不含已被其他玩家选走的武将。
- 测试房可使用调试指定的武将，不受正式房随机候选池限制。
...
- 正式房选将、锦囊结算、超时默认选将均由服务端/引擎权威驱动；客户端仅展示 prompt 与回传选择。
- 刷新/重连后 prompt 状态以服务端 `room:state` 为准。

### 5.4 与前置 REQ 的冲突处理

- 若 REQ-002 正式房 `startGame` 内直接 `assignRandomGenerals` + `engine.start()`，本期改为正式房 `selecting` 阶段，**删除正式房开局自动随机武将逻辑**。
- 测试房快捷开局逻辑保留，不强制套用正式房选将阶段。
- 玩家标识展示逻辑调整：非对局日志场景默认昵称；对局动作日志默认武将名。
...
| 可观测 | 房间重连/重进、正式房选将完成、无懈链→主体结算 记录 info 日志 |
| 无障碍 | 注册确认密码、聊天输入框、选将候选卡支持 label + Enter 提交 |
...
| AC-UX-02 | 正式房对局进行中刷新，恢复对局 Sheet 且 prompt 与断线前一致 | R-UX-01 |
| AC-UX-03 | 从游戏 Sheet 切回大厅 Sheet，房间列表本人房间显示「返回」，点击后回到原座位 | R-UX-02 |
| AC-UX-04 | 正式房选将中刷新，恢复 `room.status='selecting'` 与当前选将候选/倒计时 | R-UX-01 / R-GAME-01 |
| AC-ACCT-01 | 首页、房间聊天、房间列表、大厅表格、状态栏均显示昵称，不出现邮箱 | R-ACCT-01 |
| AC-ACCT-02 | 注册时两次密码不一致，前端拦截且不调用 API | R-ACCT-03 |
| AC-ACCT-03 | 修改昵称后，聊天与新发的 room:state 同步新昵称 | R-ACCT-02 |
| AC-ACCT-04 | 对局动作日志主要使用武将名，例如「关羽使用【杀】」 | R-ACCT-04 |
...
| AC-GAME-01 | 正式房 2 人局：主公 5 选 1，反贼 3 选 1，选完后各 4 张手牌开局 | R-GAME-01/02 |
| AC-GAME-02 | 正式房选将超时 180s 自动选候选列表第一张 | R-GAME-03 |
| AC-GAME-03 | 选将候选卡展示武将名、技能、体力/血量 | R-GAME-03 |
| AC-SBX-01 | 测试房仍可自定义角色/武将并快速进入测试流程，不强制走正式房选将 | R-SBX-01 |
...
- [x] **清晰**：每条需求有可执行判据（5min 保坐、180s 选将超时、候选 index=0、3% 像素、7 项锦囊清单）。
- [x] **完整**：覆盖刷新/误回、昵称、日志、注册、引擎断链、UI、正式房选将、测试房保留、聊天；承接 001/002 未完成 P0。
- [x] **价值**：对齐 §1.3 七项成功指标。
- [x] **优先级**：本轮以体验修复、规则闭环与正式房流程补齐为主，P0 占比高属必要。
...
| 条件 | v2 决议 | 落点 |
|---|---|---|
| `room:rejoin` 与 `room:join` 复用策略 | 复用 `room:join`，不新增产品必需 `room:rejoin` | §4.1 |
| 选将阶段字段命名 | 正式房使用 `room.status='selecting'` | §4.5 |
| sandbox 是否走选将 | 测试房保留原逻辑，不强制正式房选将 | §4.6 |
| 选将超时默认第一张 | 正式房 180s 超时，默认候选列表 `index=0` | §4.5 / §7 |
| 候选武将展示信息 | 展示名称、技能、体力/血量 | §4.5 / §7 |
| 对局日志昵称/武将名展示格式 | 对局动作日志以武将名为主；其他玩家身份场景用昵称 | §4.2 |
| 对局区 3% 像素基线 | 新增 game-sheet 基线，范围含正式房等待/选将/对局 | §4.7 |

### v2 三方复核

| 评审 | 判定 | 文档 |
|---|---|---|
| frontend-feasibility | pass | [frontend-feasibility.v2.md](../review/frontend-feasibility.v2.md) |
| backend-feasibility | pass | [backend-feasibility.v2.md](../review/backend-feasibility.v2.md) |
| qa-testability | pass | [qa-testability.v2.md](../review/qa-testability.v2.md) |
...
**REQ-2026-003 · v2**：在 001/002 基础上追加——

- 客户端 sessionStorage 房间上下文 + 房间列表「返回」高亮，重进复用 `room:join`
- 全局昵称展示 + 对局日志武将名主展示 + 改昵称 + 注册确认密码
- 引擎无懈链→主体结算断链修复 + 7 项锦囊排查
- 对局 Sheet 单元格铺满（等待/选将/对局）
- 正式房开局选将阶段（主公 5/他人 3，180s，默认 index=0）
- 测试房保留原自定义角色/武将快捷逻辑
- 房间 ChatPanel 底部输入框

影响面：`client`（App/store/ChatPanel/LobbyGrid/BattleGrid/LoginDialog/Ribbon）、`server`（room/auth/gateway）、`packages/engine`（card-play-service/identity/room 阶段机）、`docs/qa`（新增用例 UX/SEL/SBX/ENG 系列）。
...
| 章节 | 变更 |
|---|---|
| 顶部元信息 | 版本升为 v2；SSOT 指向 v2 |
| §1.3 | 选将成功指标从 60s 改为 180s；拆分昵称一致性与对局日志可读性 |
| §3.2 | 新增 R-ACCT-04 / R-GAME-03 / R-SBX-01 |
| §4.1 | 明确重进/刷新/保坐重绑复用 `room:join` |
| §4.2 | 明确非对局场景展示昵称，对局动作日志以武将名为主 |
| §4.5 | 明确正式房 `room.status='selecting'`、候选信息展示、180s 超时默认 `index=0` |
| §4.6 | 明确测试房保留原逻辑，不强制进入正式房选将 |
| §7 | 新增选将中刷新、候选信息展示、测试房保留原逻辑验收 |
| §9 | 增加 v2 产品消解与三方复核结论 |
