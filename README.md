...
| 配置驱动引擎 `@tk/engine` | ✅ | `SangokushiEngine`：准备/判定/摸牌/出牌/弃牌、杀闪桃、锦囊、濒死救助、受伤后技能、选区域牌 |
| 完整身份局（在线多人） | 🚧 | Gateway 已接线，规则与验收进行中 |
...
8. 部分处理中弹窗支持先关闭后从顶部提示条继续打开；弃牌弹窗关闭后会返回出牌阶段继续操作
9. 支持：【杀】/【闪】响应、濒死求【桃】、伤害与【反馈】【奸雄】等受伤后技能、准备阶段/摸牌前技能询问、过河拆桥/顺手牵羊选区域牌（手牌匿名+乱序，日志不展示暗牌牌名）、弃牌阶段、改判、延时锦囊按花色判定等
10. 已接线【观星】【裸衣】等阶段技能；【桃】出牌阶段仅可对自己且受伤时使用，濒死救助另行结算

### 正式房间等待大厅

1. **创建房间** 或 **加入** 8 位房间号
2. 点击 Ribbon **准备**，或在表格 **本人行「准备」列**（蓝色下划线）单击，或公式栏输入 `/ready`
3. 全员准备后，**房主** 点击 **开始** 进入对局战场表格
4. 测试房 `70755712` 禁用 Ribbon「准备」，需添加虚拟角色后 **模拟开局**

### 测试房断线重连

- 刷新页面后再次进入 `70755712`，按**昵称**匹配已断线的真人玩家，不会重复创建「表格用户」
- 对局进行中重连时，引擎内玩家 id 与 prompt 上下文会同步迁移
- 正式房等待中刷新后，`auth:hello` 与 `room:state` 会回对齐 `playerId`，准备操作仍可用
